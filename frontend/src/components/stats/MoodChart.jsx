...
function MoodChart({ data, chartType = 'line', title }) {
  ...
  const filteredData = sortedData.filter(entry => entry.mood_rating !== undefined);
  ...
  const points = filteredData.map((entry, index) => {
    ...
    const y = svgHeight - padding.bottom - ((entry.mood_rating - 1) * yScale);
    return { x, y, value: entry.mood_rating };
  });
